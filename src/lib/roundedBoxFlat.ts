...
function RoundedBoxFlat(w: number, h: number, d: number, r: number, s: number) {
  const pi2 = Math.PI * 2;
  ...
  function makeFronts(n: number, side: number, idx: number) {
    const d0 = side === 1 ? 0 : 1;
    const d1 = side === 1 ? 1 : 0;
  ...
  function makeFrame(n: number, sidx: number, sif: number, sib: number) {
